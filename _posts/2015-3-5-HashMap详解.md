...
![继承关系图](http://qn.bingying.online/img_map.png)

下面针对各个实现类的特点做下说明：

​	(1)HashMap：他根据键的hashcode值来存储数据，大多数情况下可以直接定位到他的值，因而具有很快的访问速递，但是遍历的顺序却是不确定的。HashMap最多只允许一条记录的键为null（使用注意：当你的键为null时获取键的hashcode会报错,只允许一条为null，即会键值覆盖）允许多条记录的值为null。HashMap非线程安全，即任一时刻可以有多个线程同时写HashMap，可能会导致数据的不一致，如果需要满足线程安全，可以使用Conllection的synchronizedMap方法使HashMap具有线程安全的能力，或者使用ConcurrentHashMap。
...
在使用过程中用的最频繁的还是HashMap，下面结合源码，从存储结构、常用方法分析、扩容以及安全性等方面深入讲解HashMap的工作原理

###内部实现

HashMap最常用的就是put和get方法，put进去的数据即键值对是如何在内存中存方法，即他的存储结构，我们来解下他的存储结构和功能实现方法。
...
我们需要了解：数据底层具体存储的是什么？这样的存储方式有什么优点呢？
...
![img](https://pic3.zhimg.com/80/58e67eae921e4b431782c07444af824e_hd.png)
...
![img](https://pic3.zhimg.com/80/544caeb82a329fa49cc99842818ed1ba_hd.png)
...
![img](https://pic4.zhimg.com/80/fa10635a66de637fe3cbd894882ff0c7_hd.png)

注意，Thread1的 e 指向了key(3)，而next指向了key(7)，其在线程二rehash后，指向了线程二重组后的链表。

线程一被调度回来执行，先是执行 newTalbe[i] = e， 然后是e = next，导致了e指向了key(7)，而下一次循环的next = e.next导致了next指向了key(3)。

![img](https://pic4.zhimg.com/80/d39d7eff6e8e04f98f5b53bebe2d4d7f_hd.png)
...
![img](https://pic3.zhimg.com/80/2fd11382221dfa34ab61fbee11adba3a_hd.png)
...
来源：美团技术团队  https://zhuanlan.zhihu.com/p/21673805